...
class AutoSVR:
    """
    自动优化的支持向量机回归 (SVR) 模型 - 非线性
    功能：
    1. 自动标准化数据 (StandardScaler)
    2. 使用网格搜索 (GridSearchCV) 自动选择最佳超参数 (C, gamma, epsilon)
    3. 使用 RBF 核函数处理非线性关系
    """
    ...
    def fit(self, X, y):
        """训练 SVR 模型"""
        # 1. 数据标准化 (SVR对尺度非常敏感，必须标准化)
        X_scaled = self.X_scaler.fit_transform(X)

        # y 需要 reshape 为 (-1, 1) 进行 scaling，然后再 ravel() 变回一维向量供 SVR 使用
        if len(y.shape) == 1:
            y = y.reshape(-1, 1)
        y_scaled = self.y_scaler.fit_transform(y).ravel()

        # 2. 定义参数网格
        # C: 惩罚系数，越大越容易过拟合，越小泛化越好但可能欠拟合
        # gamma: 核函数系数，控制RBF核的宽度
        # epsilon: 不敏感损失函数的宽度
        param_grid = {
        ...
        print(f"  正在通过 {self.cv_splits} 折交叉验证寻找最佳 SVR 参数...", end='', flush=True)
        ...
        print(f" 完成。")
        print(
            f"  最佳参数: C={self.best_params_['C']}, gamma={self.best_params_['gamma']}, eps={self.best_params_['epsilon']}")
...
def plot_single_catechin_results(y_true, y_pred, catechin_name, dataset_name="测试集"):
    ...
    fig.suptitle(f'{catechin_name} - {dataset_name}回归分析 (SVR)', fontsize=16, fontweight='bold')
...
def train_single_catechin_model(X, y, catechin_index, catechin_name, variety, maturity,
    ...
    """训练单个儿茶素物质的 SVR 模型"""
    print(f"\n{'=' * 60}")
    print(f"训练 {catechin_name} 模型 (SVR)")
    print(f"{'=' * 60}")
    ...
    # =========================================================
    # 替换部分：使用 AutoSVR 替代 AutoPLSR
    # =========================================================
    # cv_splits=5 表示使用 5折交叉验证来寻找最佳参数
    model = AutoSVR(cv_splits=5)
    ...
    # 格式化最佳参数字符串以便保存
    best_params_str = f"C={model.best_params_['C']}, g={model.best_params_['gamma']}, eps={model.best_params_['epsilon']}"
    ...
    try:
        ...
        for result in all_results:
            summary_data.append({
                ...
                '最佳参数': result['best_params']  # 记录SVR最佳参数
            })
        ...
        output_dir = "svr_catechin_results"
