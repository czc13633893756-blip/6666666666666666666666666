...
class SpectrumPreprocessor:
    ...
    def snv_d2st(X):
        """SNV + 二阶导数"""
        X_snv = SpectrumPreprocessor.snv(X)
        return SpectrumPreprocessor.second_derivative(X_snv)

    @staticmethod
    def apply_preprocessing(X, method='snv_d2st'):
        """应用指定的预处理方法"""
        preprocess_methods = {
            'snv_d1st': SpectrumPreprocessor.snv_d1st,
            'msc': SpectrumPreprocessor.msc,
            'snv_d2st': SpectrumPreprocessor.snv_d2st
        }
...
def train_single_catechin_model(X, y, catechin_index, catechin_name, variety, maturity,
    ...
    # 应用SNV+D2st预处理
    X_processed = SpectrumPreprocessor.apply_preprocessing(X, 'snv_d2st')
...
def main():
    ...
    print("=" * 60)
    print("茶叶儿茶素含量预测模型 (深度学习 + SNV+D2st预处理)")
    ...
    try:
        ...
        # 4. 为每个儿茶素物质单独训练模型
        print(f"\n4. 开始为每个儿茶素物质单独训练深度学习模型 (使用SNV+D2st预处理)...")
        print(f"预处理方法: SNV + 二阶导数")
        ...
        # 6. 保存结果到CSV文件
        output_dir = "catechin_dl_results_aligned_by_catechin"
        ...
        ax.set_xlabel('儿茶素物质')
        ax.set_ylabel('R²')
        ax.set_title('深度学习模型 - 各儿茶素物质的R²比较')
        ax.set_xticks(x_pos)
        ...
        # 统计分层信息
        train_variety_counts = stratify_info[stratify_info['数据集划分'] == '训练集']['品种'].value_counts()
        test_variety_counts = stratify_info[stratify_info['数据集划分'] == '测试集']['品种'].value_counts()

        train_maturity_counts = stratify_info[stratify_info['数据集划分'] == '训练集']['成熟度'].value_counts()
        test_maturity_counts = stratify_info[stratify_info['数据集划分'] == '测试集']['成熟度'].value_counts()
