...
class SpectrumPreprocessor:
    ...
    @staticmethod
    def apply_preprocessing(X, method='snv_d1st'): # 提取品种、成熟度和样本ID'snv_d1st'
        """应用指定的预处理方法"""
        preprocess_methods = {
...
def train_single_catechin_model(X, y, catechin_index, catechin_name, variety, maturity,
    ...
    # 应用SNV+D1st预处理
    X_processed = SpectrumPreprocessor.apply_preprocessing(X, 'snv_d1st')#(yuchuli修改地方snv_d1st)
...
def main():
    ...
    try:
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
            f.write("茶叶儿茶素含量预测深度学习模型报告 (SNV+D1st预处理 + 分层抽样)\n")
            f.write("=" * 60 + "\n\n")
