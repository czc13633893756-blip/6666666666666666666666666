...
from sklearn.model_selection import train_test_split, cross_val_score, KFold, StratifiedShuffleSplit, cross_val_predict, \
    GridSearchCV
# 替换：使用 sklearn 自带的 GradientBoostingRegressor，无需安装 xgboost
...
class AutoGradientBoosting:
    """
    自动优化的 Sklearn GradientBoosting 回归模型
    替代 XGBoost，无需额外安装库
    """
    ...
    def fit(self, X, y):
        ...
        param_grid = {
            ...
            'subsample': [0.8, 1.0],
            'max_features': ['sqrt', None]  # 'sqrt' 类似于 colsample_bytree < 1.0
        }

        print(f"  正在通过 {self.cv_splits} 折交叉验证寻找最佳 GB 参数...", end='', flush=True)
...
def plot_single_catechin_results(y_true, y_pred, catechin_name, dataset_name="测试集"):
    ...
    fig.suptitle(f'{catechin_name} - {dataset_name}回归分析 (GradientBoosting)', fontsize=16, fontweight='bold')
    ...
    metrics_text = f"评估指标汇总 (GradientBoosting):\n\n"
...
def train_single_catechin_model(X, y, catechin_index, catechin_name, variety, maturity,
    ...
    """训练单个儿茶素物质的 GradientBoosting 模型"""
    print(f"\n{'=' * 60}")
    print(f"训练 {catechin_name} 模型 (GradientBoosting)")
    print(f"{'=' * 60}")
    ...
    # =========================================================
    # 替换部分：使用 AutoGradientBoosting 替代 AutoXGBoost
    # =========================================================
    model = AutoGradientBoosting(cv_splits=5, random_state=random_state)
...
def main():
    ...
    print("=" * 60)
    print("茶叶儿茶素含量预测模型 (GradientBoosting - sklearn)")
    print("预处理: SNV + 一阶导数")
    print("=" * 60)
    ...
    try:
        ...
        for result in all_results:
            summary_data.append({
                ...
                '最佳参数': str(result['model'].best_params)
            })

        summary_df = pd.DataFrame(summary_data)
        summary_df = summary_df.sort_values('测试集R²', ascending=False)

        print("\n模型性能汇总表 (GradientBoosting):")
        print(summary_df.to_string(index=False))

        output_dir = "gb_catechin_results"
