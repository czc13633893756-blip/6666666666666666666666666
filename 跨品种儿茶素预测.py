...
def calculate_rpd(y_true, y_pred):
    ...
    RPD (Relative Percent Difference) = 标准差 / RMSE
    评价标准：
    RPD > 2.5: 模型预测能力优秀
    2.0 < RPD <= 2.5: 良好
    1.8 < RPD <= 2.0: 可接受
...
def create_variety_based_split(X, variety, train_varieties_count=10, test_varieties_count=2, random_state=42):
    """创建基于品种的数据集划分：10个品种作为训练集，2个品种作为测试集
    ...
    variety : array-like
        品种标签
    train_varieties_count : int
        训练集品种数量
    test_varieties_count : int
        测试集品种数量
    random_state : int
        随机种子
    ...
    返回:
    ----------
    train_idx, test_idx : array
        训练集和测试集的索引
    """
    # 获取所有唯一品种
    unique_varieties = np.unique(variety)
    print(f"总品种数量: {len(unique_varieties)}")

    # 检查品种数量是否足够
    if len(unique_varieties) < train_varieties_count + test_varieties_count:
        raise ValueError(f"品种数量不足: 有{len(unique_varieties)}个品种，但需要{train_varieties_count + test_varieties_count}个品种")

    # 设置随机种子
    np.random.seed(random_state)

    # 随机选择训练集和测试集品种
    shuffled_varieties = np.random.permutation(unique_varieties)
    train_varieties = shuffled_varieties[:train_varieties_count]
    test_varieties = shuffled_varieties[train_varieties_count:train_varieties_count + test_varieties_count]

    print(f"训练集品种 ({len(train_varieties)}个): {train_varieties}")
    print(f"测试集品种 ({len(test_varieties)}个): {test_varieties}")

    # 根据品种分配训练集和测试集索引
    train_idx = []
    test_idx = []

    for i, var in enumerate(variety):
        if var in train_varieties:
            train_idx.append(i)
        elif var in test_varieties:
            test_idx.append(i)
    ...
    print(f"训练集比例: {len(train_idx) / len(variety):.2%}")
    print(f"测试集比例: {len(test_idx) / len(variety):.2%}")

    # 打印每个品种的样本数量
    print(f"\n训练集各品种样本数量:")
    for var in train_varieties:
        count = np.sum(variety[train_idx] == var)
        print(f"  {var}: {count} 个样本")

    print(f"\n测试集各品种样本数量:")
    for var in test_varieties:
        count = np.sum(variety[test_idx] == var)
        print(f"  {var}: {count} 个样本")

    return train_idx, test_idx, train_varieties, test_varieties
...
def train_single_catechin_model(X, y, catechin_index, catechin_name, variety, maturity,
    ...
    print(f"\n测试集品种分布:")
    unique_test_variety, counts_test_variety = np.unique(test_variety, return_counts=True)
    ...
    print(f"\n测试集成熟度分布:")
    unique_test_maturity, counts_test_maturity = np.unique(test_maturity, return_counts=True)
...
def main():
    ...
    print("=" * 60)
    print("茶叶儿茶素含量预测模型 (深度学习 + SNV+D1st预处理)")
    print("使用基于品种的划分：10个品种训练，2个品种测试")
    ...
    try:
        ...
        # 3. 创建基于品种的数据集划分：10个品种训练，2个品种测试
        print(f"\n3. 创建基于品种的数据集划分...")
        train_idx, test_idx, train_varieties, test_varieties = create_variety_based_split(
            X_aligned, aligned_variety,
            train_varieties_count=10, test_varieties_count=2,
            random_state=42
        )

        # 修复：将品种数组转换为字符串列表
        train_varieties_str = [str(var) for var in train_varieties]
        test_varieties_str = [str(var) for var in test_varieties]

        print(f"训练品种: {train_varieties_str}")
        print(f"测试品种: {test_varieties_str}")

        # 4. 为每个儿茶素物质单独训练模型
        print(f"\n4. 开始为每个儿茶素物质单独训练深度学习模型 (使用SNV+D1st预处理)...")
        print(f"预处理方法: SNV + 一阶导数")
        print(f"数据集划分: 10个品种训练，2个品种测试")
        print(f"训练品种: {train_varieties_str}")
        print(f"测试品种: {test_varieties_str}")
        ...
        # 6. 保存结果到CSV文件
        output_dir = "catechin_dl_results_variety_split"
        ...
        ax.set_xlabel('儿茶素物质')
        ax.set_ylabel('R²')
        ax.set_title('深度学习模型 - 各儿茶素物质的R²比较 (10品种训练，2品种测试)')
        ax.set_xticks(x_pos)
        ...
        ax.set_xlabel('儿茶素物质')
        ax.set_ylabel('RPD')
        ax.set_title('深度学习模型 - 各儿茶素物质的测试集RPD比较')
        ax.set_xticks(x_pos)
        ax.set_xticklabels(summary_df['儿茶素物质'], rotation=45, ha='right')
        ax.grid(True, alpha=0.3, axis='y')
        ...
        ax.set_xlabel('儿茶素物质')
        ax.set_ylabel('RMSE')
        ax.set_title('深度学习模型 - 各儿茶素物质的RMSE比较')
        ax.set_xticks(x_pos)
        ax.set_xticklabels(summary_df['儿茶素物质'], rotation=45, ha='right')
        ax.legend()
        ax.grid(True, alpha=0.3, axis='y')
        ...
        variety_path = os.path.join(output_dir, "variety_split_info.csv")
        variety_info.to_csv(variety_path, index=False, encoding='utf-8-sig')
        print(f"品种划分信息已保存到: {variety_path}")

        # 保存品种列表（转换为字符串）
        variety_list = pd.DataFrame({
            '训练品种': train_varieties_str + [''] * (
                        max(len(train_varieties_str), len(test_varieties_str)) - len(train_varieties_str)),
            '测试品种': test_varieties_str + [''] * (
                        max(len(train_varieties_str), len(test_varieties_str)) - len(test_varieties_str))
        })

        variety_list_path = os.path.join(output_dir, "train_test_varieties.csv")
        variety_list.to_csv(variety_list_path, index=False, encoding='utf-8-sig')
        print(f"品种列表已保存到: {variety_list_path}")

        # 统计品种信息
        train_variety_counts = variety_info[variety_info['数据集划分'] == '训练集']['品种'].value_counts()
        test_variety_counts = variety_info[variety_info['数据集划分'] == '测试集']['品种'].value_counts()

        train_maturity_counts = variety_info[variety_info['数据集划分'] == '训练集']['成熟度'].value_counts()
        test_maturity_counts = variety_info[variety_info['数据集划分'] == '测试集']['成熟度'].value_counts()

        print("\n品种划分统计:")
        print("训练集品种分布:")
        for variety, count in train_variety_counts.items():
            print(f"  {variety}: {count} 个样本 ({count / len(train_idx):.1%})")

        print("\n测试集品种分布:")
        for variety, count in test_variety_counts.items():
            print(f"  {variety}: {count} 个样本 ({count / len(test_idx):.1%})")

        print("\n训练集成熟度分布:")
        for maturity, count in train_maturity_counts.items():
            print(f"  {maturity}: {count} 个样本 ({count / len(train_idx):.1%})")

        print("\n测试集成熟度分布:")
        for maturity, count in test_maturity_counts.items():
            print(f"  {maturity}: {count} 个样本 ({count / len(test_idx):.1%})")

        # 10. 生成模型报告
        report_path = os.path.join(output_dir, "dl_model_report.txt")
        with open(report_path, 'w', encoding='utf-8') as f:
            f.write("茶叶儿茶素含量预测深度学习模型报告 (SNV+D1st预处理 + 品种划分)\n")
            f.write("=" * 60 + "\n\n")
            f.write(f"生成时间: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write(f"样本数量: {X_aligned.shape[0]}\n")
            f.write(f"特征数量: {X_aligned.shape[1]}\n")
            f.write(f"儿茶素物质数量: {y_aligned.shape[1]}\n")
            f.write(f"预处理方法: SNV + 一阶导数\n")
            f.write(f"模型类型: 多层感知机(MLP)\n")
            f.write(f"框架: PyTorch\n")
            f.write(f"数据集划分: 10个品种训练，2个品种测试\n")
            f.write(f"训练品种: {', '.join(train_varieties_str)}\n")
            f.write(f"测试品种: {', '.join(test_varieties_str)}\n")
        ...
        print(f"模型类型: 多层感知机(MLP)")
        print(f"数据集划分: 10个品种训练，2个品种测试")
        print(f"训练品种: {', '.join(train_varieties_str)}")
        print(f"测试品种: {', '.join(test_varieties_str)}")
        print(f"数据对齐: 以儿茶素数据为基准")
        ...
        print(f"  4. {output_dir}/variety_split_info.csv - 品种划分信息")
        print(f"  5. {output_dir}/train_test_varieties.csv - 品种列表")
        print(f"  6. {output_dir}/dl_model_report.txt - 模型报告")
        print(f"  7. 各儿茶素物质的回归分析图")
        print(f"  8. 各儿茶素物质的损失历史图")
        print(f"{'=' * 60}")
